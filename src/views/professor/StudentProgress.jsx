    ...
    const loadStudentsProgress = async () => {
        ...
        try {
            ...
            const studentData = profiles?.map(profile => {
                ...
                // Total points
                const totalPoints = studentScores.reduce((sum, s) => sum + s.score, 0);
                ...
                ['fault_roulette', 'service_writer', 'cross_system', 'tool_selection'].forEach(gameType => {
                    ...
                    gameBreakdown[gameType] = {
                        points: gameScores.reduce((sum, s) => sum + s.score, 0),
                        games: studentPlays.filter(p => p.game_type === gameType).length,
                        best: Math.max(...gameScores.map(s => s.score), 0)
                    };
                });
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-4">
                ...
                <Card className="p-4 text-center">
                    ...
                    <div className="text-xs text-slate-500">Avg Points/Student</div>
                </Card>
            </div>
            ...
            <Card className="divide-y divide-slate-100 dark:divide-slate-800">
                {/* Table Header */}
                <div className="grid grid-cols-12 gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 text-xs font-bold text-slate-500 uppercase tracking-wider">
                ...
                {filteredStudents.length === 0 ? (
                    ...
                    filteredStudents.map((student, index) => (
                        <div key={student.id}>
                            {/* Student Row */}
                            <div
                                className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-slate-50 dark:hover:bg-slate-800/50 cursor-pointer transition-colors"
