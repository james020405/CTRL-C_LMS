...
export const isGeminiAvailable = () => true; // Always available via backend

export const generateCustomerScenario = async (system, fault, partName) => {
    try {
        return await callApi('generate-scenario', { system, fault, partName });
    } catch (error) {
...
const getFallbackScenario = (system, partName) => {
    return {
        ...
        complaint: `It's making a weird noise when I use the ${system}.`,
        isAI: false
    };
};
...
export const generateServiceCustomer = async (difficulty = 'easy') => {
    try {
        const data = await callApi('generate-customer', { difficulty });
        console.log("✅ Service customer generated via Backend API");
        return data;
    } catch (error) {
...
export const evaluateEstimate = async (customer, estimate, notes) => {
    try {
        const data = await callApi('evaluate-estimate', { customer, estimate, notes });
        console.log("✅ Estimate evaluated via Backend API");
        return data;
    } catch (error) {
        console.warn("⚠️ API failed, using fallback:", error.message);
...
export const askCustomerQuestion = async (customer, question, conversationHistory = []) => {
    try {
        const data = await callApi('ask-customer', { customer, question, history: conversationHistory });
        return data.answer;
    } catch (error) {
        console.warn("⚠️ API failed, using fallback:", error.message);
        return "Pasensya na, hindi ko naintindihan.";
    }
};

export const askTechnicianToCheck = async (customer, command) => {
    try {
        const data = await callApi('ask-technician', { customer, command });
        return data.findings;
    } catch (error) {
        console.warn("⚠️ API failed, using fallback:", error.message);
        return "Technician represents: No issues found (Offline Mode)";
    }
};

// ==========================================
// FALLBACK DATA (Minified for clarity)
// ==========================================

const FALLBACK_CUSTOMERS = {
    easy: [
        { name: "Tita Marie", vehicle: "2020 Toyota Fortuner", complaint: "Oil change reminder light on", mood: "Friendly", budget: 25000, dialogue_start: "Hi! I think it's time for my regular service.", technicianReport: "Vehicle due for 40,000km service.", actualProblem: "Routine maintenance", correctParts: ["Engine Oil", "Oil Filter"], idealEstimateRange: { min: 8000, max: 15000 }, recommendedLaborHours: 1.5 }
    ]
};

const getFallbackServiceCustomer = (difficulty = 'easy') => {
    const customers = FALLBACK_CUSTOMERS.easy; // Simplified fallback
    const customer = customers[0];

    // Generate simple options
    const options = [
        { id: 'A', parts: [{ name: 'Oil', cost: 2000 }], laborHours: 1, laborRate: 500, explanation: 'Correct' },
        { id: 'B', parts: [{ name: 'Oil', cost: 5000 }], laborHours: 2, laborRate: 500, explanation: 'Overpriced' },
        { id: 'C', parts: [{ name: 'Water', cost: 50 }], laborHours: 0.5, laborRate: 500, explanation: 'Wrong' },
        { id: 'D', parts: [{ name: 'Brake Pads', cost: 3000 }], laborHours: 1, laborRate: 500, explanation: 'Wrong Part' }
    ];

    return { ...customer, estimateOptions: options, correctAnswer: 'A' };
};

const getFallbackEvaluation = (customer, estimate, notes) => {
    return {
        outcome: 'Accepted',
        message: "Sige, okay na yan via offline fallback.",
        feedback: "Offline mode fallback used.",
